...
function showActPageAssessment_new(activity, uact){
	...
	if (!activity.assessment || activity.assessment.enabled == '0'){
		...
		if (uact.uact_coordinator == 1){
			jtr.show();
			showActPageAssessment_coordinator1(jtr.find('.my_datatable'), activity.assessment.assessments);
		} else {
			jtr.hide();
		}
...
function showActPageAssessment_coordinator1(jtbl, assessments){

	console.debug('showActPageAssessment_coordinator1', jtbl, assessments);
/*	
...
*/
	...
	for (var index in assessments){
		...
		;
			...
			ass_id,
//			name,
			'<button type="button" class="btn btn-sm btn-list-edit" onclick=\'showActPageAssessment_coordinator2_ios(getIdFromRow(this), this)\' data-toggle="tooltip" title="View">'
			...
			100,			
			'<button type="button" class="btn btn-sm btn-list-edit" onclick=\'showActPageAssessment_coordinator2(getIdFromRow(this), this)\' data-toggle="tooltip" title="View">'
				+ '<i class="glyphicon glyphicon-eye-open"></i>'
			+ '</button>',
...
function showActPageAssessment_coordinator2(ass_id, obj){

	console.debug('showActPageAssessment_coordinator2', ass_id);
	...
	if (g_curr_inline_jtr){
		...
		;
		if (ass_id2 == ass_id && dt_type == 'assessment_coordinator1'){
	...
	var
		...
		assessment = getAssessmentByAssID(g_curr_activity, ass_id)
		participants_ass = assessment.participants
	;
	//console.debug(assessment); return;
	var jobj = $(obj);
	//console.debug(jobj);
	...
	//console.debug(jtr);
	var s = '<table id="example888" class="my_datatable display responsive nowrap" dt_type="assessment_coordinator2" style="padding-left:20px; width:100%">'
					+ '<thead>'
						+ '<tr>'
							+ '<th>User ID</th>'
							+ '<th>Particpants</th>'
...
//			{	targets: [ 3 ],	width:1, orderable: false},
	...
	if (participants_ass){
		...
		jtbl.find('.tbl_assessors').click(function(){
			showActPageAssessment_coordinator3(ass_id, $(this));	// this => user_id
		});
	}
...
function showActPageAssessment_coordinator2_ios(ass_id, obj){

	console.debug('showActPageAssessment_coordinator2_ios', ass_id);
	...
	//console.debug(jtr);
	var s = '<table id="example" ass_id="' + ass_id + '" class="my_datatable display responsive nowrap" dt_type="assessment_coordinator2" style="padding-left:20px; width:100%">'
					+ '<thead>'
						+ '<tr>'
							+ '<th>User ID</th>'
							+ '<th>Particpants</th>'
...
//			{	targets: [ 3 ],	width:1, orderable: false},
	...
	if (participants_ass){
		for (var i = 0; i < participants_ass.length; i++){
			...
			if (!participant_user){
				...
				var date = participant_ass.date ? getDateWithoutTime(participant_ass.date) : '',
					...
					nRated = 3,
					//list = createAssessorsList(participant_ass.assessors, nRated)
					button = '<button type="button" class="btn btn-sm btn-list-edit" onclick=\'showActPageAssessment_coordinator3($(this))\' data-toggle="tooltip" title="View">'
							+ '<i class="glyphicon glyphicon-eye-open"></i>'
						+ '</button>'					
		...
		jtbl.find('.tbl_assessors').click(function(){
			//showActPageAssessment_coordinator3(ass_id, $(this));	// this => user_id
			showActPageAssessment_coordinator3($(this));	// this => user_id
		});
	}
...
//alert("33");
		var 
			//skill = getSkillByName(g_curr_activity.assessment.skills, skill_name),
			jtr = jobj.closest('tr'),
		...
		if (participant_ass && participant_ass.assessors && participant_ass.assessors.length){
			...
			console.debug('showActPageAssessment_coordinator3', participant_id, participant_ass, assessors);
			
			var jdiv = $('<div/>');
			var s = 
				'<span class="subsection_header">Markings for ' + participant_name + '\'s ' + assessment_name + '</span>'
				+ '<table id="dt_level3" class="my_datatable display responsive nowrap" dt_type="assessment_coordinator3" style="width:100%">'
						+ '<thead>'
							+ '<tr>'
								+ '<th>Assessors</th>'
								+ '<th>Date</th>'
...
function showActPageAssessment_coordinator3(jobj){
	var 
		jtr = jobj.closest('tr');
	...
	var jtbl = $(jtr.closest('table')),
		...
		participant_name = $('<div>' + cols[1] + '</div>').text()
	;
	console.debug('showActPageAssessment_coordinator3', ass_id);
	
	//openAssessment(ass_id, 'coordinator', participant_id, participant_name);			
}
...
function getUserImgSrc(img_id){
	//console.debug('getUserImgSrc', img_id);
