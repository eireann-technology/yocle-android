...
function remember(uid, pwd) {
 if ($('#remember').is(':checked')) {
		var username = uid;
		var password = pwd;
		
		if(!rememberme(username+":"+password+":1")) {	
			createCookie('username', username, 365);	
			createCookie('password', password, 365);	
			createCookie('remember', "1", 365);	
		}
	} 
	else {
		if(!rememberme("::0")) {		
			createCookie('username', "", 365);	
			createCookie('password', "", 365);	
			createCookie('remember', "0", 365);	
		}
	}
}
...
if(typeof remember === 'undefined'){ checked = false; }
else if(remember=="1") checked = true;
var username = readCookie('username');
var password = readCookie('password');
// autofill the fields
$('#uid').attr("value", username);
...
function showPortfolio(type) {
 if(isMobile.Android()) {
		if(typeof app != "undefined")	{
		  log2Google("Portfolio", "showportfolio type="+type);
			app.showPortfolio(type) ;		
			return;
		}
	}	
	else if(isMobile.iOS()) {
	  log2Google("Portfolio", "showportfolio type="+type);
		window.location = "http://showportfolio.adiai.com/"+type;
		return;
	}

}
function showErrorDialog(errnum, m1, m3, m6, m12) {
 if(isMobile.Android()) {
		if(typeof app != "undefined")	{
			app.showErrorDialog(errnum, m1, m3, m6, m12) ;		
			return;
		}
	}	
	else if(isMobile.iOS()) {
		window.location = "http://showerrordialog.adiai.com/"+errnum+"/"+m1+"/"+m3+"/"+m6+"/"+m12;
		return;
	}
}
